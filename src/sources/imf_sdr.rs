    ...
    pub async fn trading_pairs(&self, pair: &Pair) -> Result<Response, Error> {
        ...
        let mut imf_sdr = csv::ReaderBuilder::new()
            ...
            for result in imf_sdr.records() {
                ...
                match row {
                    Ok(imf_sdr_row) => if imf_sdr_row.currency == pair.0.imf_long_name() {response_row = Some(imf_sdr_row); break;},
                    Err(_e) => continue,
                };
             ...
             match response_row{
                 Some(resp) => Ok(Response{price:resp.price_1}),
